...
def dashboard(request):
    ...
    try:
        organizacion = Organizacion.objects.get(admin=user)
    except Organizacion.DoesNotExist:
        organizacion = None
    destinatarios = Destinatario.objects.filter(organizacion=organizacion) if organizacion else []
...
def panel_organizacion(request):
    ...
    """
    try:
        organizacion = Organizacion.objects.get(admin=request.user)
    except Organizacion.DoesNotExist:
    ...
    destinatarios = Destinatario.objects.filter(organizacion=organizacion)
    return render(request, 'panel_organizacion.html', {
        'organizacion': organizacion,
        'destinatarios': destinatarios,
        'dominio': dominio,
...
def historial_informes(request):
    ...
    if user.is_superuser:
        informes = InformeEnviado.objects.select_related('empresa').order_by('-fecha_envio')
    else:
        try:
            organizacion = Organizacion.objects.get(admin=user)
            empresa = Empresa.objects.filter(nombre__iexact=organizacion.nombre).first()
            if empresa:
                informes = InformeEnviado.objects.filter(empresa=empresa).select_related('empresa').order_by('-fecha_envio')
            else:
                informes = []
        except Organizacion.DoesNotExist:
            informes = []
