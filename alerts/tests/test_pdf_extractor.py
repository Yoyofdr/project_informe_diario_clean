...
class TestPDFExtractor(TestCase):
    ...
    def test_fallback_mechanism(self):
        """Test del mecanismo de fallback entre métodos (mockeado en instancia, pipeline usa los mocks)"""
        mock_pypdf2 = Mock(side_effect=Exception("PyPDF2 failed"))
        mock_pdfminer = Mock(return_value="abc")  # Muy corto
        mock_ocr = Mock(return_value="Este es un texto válido extraído por OCR con suficientes palabras para pasar la validación.")
        self.extractor.methods_priority = [
            ('pypdf2', mock_pypdf2),
    ...
    def test_extract_with_pypdf2_success(self, mock_pypdf2):
        """Test de extracción exitosa con PyPDF2 (mockeado)"""
        mock_pypdf2.return_value = "Texto extraído con PyPDF2 válido para test."
        pdf_content = b"fake pdf content"
        text = self.extractor._extract_with_pypdf2(pdf_content, max_pages=2)
        self.assertIn("PyPDF2 válido", text)

    @patch('alerts.services.pdf_extractor.extract_text')
    def test_extract_with_pdfminer_success(self, mock_extract_text):
        """Test de extracción exitosa con PDFMiner (mockeado)"""
        expected_text = "Contenido extraído con PDFMiner. Este método es más robusto para PDFs complejos."
        mock_extract_text.return_value = expected_text
        pdf_content = b"fake pdf content"
        text = self.extractor._extract_with_pdfminer(pdf_content, max_pages=2)
    ...
    @patch('pdf2image.convert_from_bytes')
    def test_extract_with_ocr_success(self, mock_convert, mock_ocr):
        """Test de extracción exitosa con OCR (mockeado en instancia, solo verifica resultado)"""
        self.extractor._extract_with_ocr = Mock(return_value="Texto extraído mediante OCR desde imagen escaneada del documento.")
        pdf_content = b"fake pdf content"
        text = self.extractor._extract_with_ocr(pdf_content, max_pages=2)
        self.assertIn("OCR", text)

    # Nota: Para testear el pipeline completo con archivos PDF reales, se recomienda un test de integración aparte con archivos válidos en el repo.