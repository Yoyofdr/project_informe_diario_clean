#!/usr/bin/env python
"""
Script para generar el informe del Diario Oficial organizado por secciones
"""
import os
import sys
import django
...
def generar_informe_html(fecha):
    """Genera el informe en formato HTML organizado por secciones"""
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        fecha_hoy = "17-07-2025"  # datetime.now().strftime("%d-%m-%Y")
    
    html = generar_informe_html(fecha_hoy)
    
    # Guardar el HTML
    filename = f"informe_secciones_{fecha_hoy.replace('-', '_')}.html"
    with open(filename, "w", encoding="utf-8") as f:
        f.write(html)
    
    print(f"\nInforme generado: {filename}")
    print(f"El informe está organizado por secciones: Normas Generales, Normas Particulares y Avisos Destacados")